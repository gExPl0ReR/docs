...
The **EDIS Global Management API** lets you manage active VPS services in your account.
...
The Management API does not use static API keys. There is no API key to create or copy.

Instead, you authenticate with your **WHMCS credentials**. WHMCS is the billing software that powers the EDIS Global Client Portal. Your WHMCS credentials are the same email and password you use to sign in to the Client Portal.

Each auth request returns temporary session values (`api_host`, `kvm_id`, `signature`, `valid_until`) that you pass to all other management endpoints. Create a new auth session when the previous one expires.
...
<Tabs>
  ...
  <Tab title="Developer Version">
    ...
    **Important output fields:**
    ...
    - [Create AUTH Session](/management-api-reference/authentication/create-auth-session)\
    ...
    - [HARD RESET](/management-api-reference/power-management/hard-reset)\
      Performs an immediate hard reboot of the VPS (restarts hypervisor process)
    - [POWER STATE](/management-api-reference/power-management/power-state)\
      Returns the current power state of the VPS (e.g., powered on or powered off).
    - [POWER ON](/management-api-reference/power-management/power-on)\
      Powers on a VPS that is currently shut down.
    - [POWER OFF](/management-api-reference/power-management/power-off)\
    ...
    - [List Available ISO images for installation](/management-api-reference/installation/iso-installation)\
      Returns a list of ISO images available for manual installation, recovery, or rescue environments.
    - [List of Autoinstall OS Images](/management-api-reference/installation/get-autoinstall-images)\
      Returns available operating system images for automated installation.
    - [Reinstall fresh OS](/management-api-reference/installation/reinstall)\
      Reinstalls the VPS using an autoinstall operating system image.
    - [Mount ISO](/management-api-reference/installation/mount-iso)\
      Mounts an ISO image to the VPS for manual installation or recovery.
    - [Umount ISO](/management-api-reference/installation/unmount-iso)\
      Removes the currently mounted ISO from the VPS.
    - [Provide Custom ISO](/management-api-reference/installation/download-custom-iso)\
      Downloads and mounts a custom ISO image from a specified URL.
    - [Upload and Replace QCOW2](/management-api-reference/installation/upload-and-replace-qcow2)\
    ...
    - [Enable Auto-Hydration](/management-api-reference/traffic-pool/enable-auto-hydration)\
      Selects a **100 GB**, **200 GB**, **300 GB**, or **500 GB** automatic assignment for one VPS. The assignment runs at 99% usage and requires a sufficient Traffic Pool balance.
    - [Disable Auto-Hydration](/management-api-reference/traffic-pool/disable-auto-hydration)\
      Disables automatic traffic assignment.
    - [Auto-Hydration Status](/management-api-reference/traffic-pool/get-auto-hydration-status)\
      Returns the current auto-hydration status for the VPS.
    - [Traffic Pool balance](/management-api-reference/traffic-pool/get-global-traffic-pool-size)\
      Returns the available balance in the global traffic pool.
    - [Hydration Log](/management-api-reference/traffic-pool/get-hydration-log)\
      Returns a log of traffic assignments (automatic and manual).
    - [Manually Assign Traffic (Manual Hydration)](/management-api-reference/traffic-pool/assign-traffic-resources-from-global-pool)\
    ...
    - [Retrieve Hardware Settings](/management-api-reference/settings/get-configuration)\
      Returns the current configurable hardware settings of the VPS.
    - [Save changes Hardware Settings](/management-api-reference/settings/configure-settings)\
    ...
    - [Pull a VNC screenshot](/management-api-reference/collect-information/get-vps-screenshot)\
      Captures and returns a screenshot of the VPS console via VNC.
    - [VPS Details](/management-api-reference/collect-information/get-all-vps-details)\
      Returns full VPS details, including configuration, usage, network data, and metadata.
    - [Traffic Usage](/management-api-reference/collect-information/get-traffic-usage)\
      Returns traffic usage statistics for the VPS.
    - [CPU Usage](/management-api-reference/collect-information/get-cpu-usage)\
      Returns current or recent CPU usage data.
    - [Basic Details](/management-api-reference/collect-information/get-basic-details)\
      Returns a simplified set of VPS information.
    - [Mount Details](/management-api-reference/collect-information/get-mount-details)\
    ...
    - [VNC Keymap](/management-api-reference/vnc-management/set-vnc-keyboard-language)\
      Sets the keyboard layout for the VNC console.
    - [VNC Status](/management-api-reference/vnc-management/get-vnc-status)\
      Returns the current VNC access status.
    - [Enable VNC](/management-api-reference/vnc-management/enable-vnc-access)\
      Enables VNC access to the VPS.
    - [Disable VNC](/management-api-reference/vnc-management/disable-vnc-access)\
      Disables VNC access.
    ...
    - [Get PTR](/management-api-reference/ptr/get-ptr-records)\
      Returns current reverse DNS (PTR) records.
    - [Delete PTR](/management-api-reference/ptr/delete-ptr-records)\
      Deletes a PTR record.
    - [Set PTR](/management-api-reference/ptr/set-ptr-records)\
    ...
    - [Get all Webhooks](/management-api-reference/hooks/get-all-webhooks)\
      Returns all configured webhooks for the VPS.
    - [Delete Webhook](/management-api-reference/hooks/delete-a-webhook)\
      Deletes a webhook.
    - [Traffic Monitor Webhook](/management-api-reference/hooks/set-webhook-for-traffic-monitoring)\
      Creates a webhook that triggers when a traffic threshold is reached.
    - [Callback example](/management-api-reference/hooks/example-callback-triggered)\
  ...
  <Tab title="Full End-2-End Example">
    ...
    <Info>
      A Linux reinstall usually finishes in about 5 minutes. No email with credentials is sent.
    </Info>
