---
title: Cloud-Init based VPS installation
sidebarTitle: Cloud-Init
description: "How EDIS Global moved to Cloud-Init and QCOW2-based VPS installations. Learn the benefits over ISO setups and how to use cloudinit_user_data on first boot."
---

import { CtaButton } from "/snippets/cta-button.mdx";

## Easy VPS installation with Cloud-Init
...
### When to supply custom user data

Custom `cloudinit_user_data` is currently supported through the Management API's [`set/reinstall` operation](/management-api-reference/installation/reinstall) with a supported image. It is **not supported in the initial Order API request**. Reinstallation replaces the existing operating system and data; back up anything you need first.

To configure a new VPS without an additional reinstall, order a standard OS with an SSH public key and apply your configuration over SSH, for example with Ansible. See [the API deployment workflow](/order-api-reference/order-api#before-you-automate).

`cloudinit_user_data` is a powerful feature of Cloud-Init that allows you to provide custom configurations and scripts that the system will execute upon initialization. This data is in the format of `#cloud-config`, which is a YAML-based configuration file.
...
The shift to Cloud-Init and .qcow2 based installations marks a significant advancement in the way EDIS Global manages its VPS offerings. By leveraging Cloud-Init, you can enjoy faster deployment, increased automation, and more customized configurations, enhancing the overall user experience and efficiency.
