...
Passwords are generated during deployment and configured directly on the server. They are never saved in our systems. If you did not receive or no longer have the initial password, it cannot be recovered.

In this case, the server must be reinstalled and a new password will be set during the installation process.

This design ensures maximum security — credentials that are never stored cannot be leaked, accessed, or compromised. ;-)