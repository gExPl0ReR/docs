...
sidebarTitle: "Subscribe to Status Updates"
...
The **EDIS Global Status API** provides real-time service health data, allowing users to integrate incident and maintenance updates into their workflows. It offers endpoints to retrieve overall status, component health (including uptime metrics), and a log of past and ongoing notices. 

Each component can be queried individually, and metrics such as uptime percentage, mean time to recovery, and mean time between failures are available for the last 30 days or custom timeframes. Users can filter for planned/unplanned events, and pagination is provided for large datasets. The system is rate-limited to 10 requests per second to ensure fair use.\
\
Integrate our status-messages into your production system with the EDIS Global[ Statuspage API](https://status.edis.global/api). 