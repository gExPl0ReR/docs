...
apt install curl -y
```
5. Disable IPv6 on Ubuntu 22.04:
```bash
...
7. During the installation process script asked about change port
```bash
...
- Generate and assign a self-sign cert for panel
- Use SSH port forwarding (connection with SSH tunnel)
- Use Let's encrypt SSL cert if you register a domain.
