---
title: "Install the 3x-ui VLESS proxy panel on Ubuntu 22.04"
sidebarTitle: "Install 3x-ui on Ubuntu 22.04"
description: "Install 3x-ui on Ubuntu 22.04 VPS to run a VLESS proxy panel for privacy, DPI bypass, and access to restricted content with a managed web interface."
---
...
<Steps>
  ...
  <Step title="During the installation, the script asks about changing the port">
    ```bash
        Would you like to customize the Panel Port settings? (If not, a random port will be applied) [y/n]: 
    ```

    Type **y** to set your own port. The port should be in the 41000-65000 range. For example: 46522
...
<Tabs>
  <Tab title="Self-sign cert" icon="expeditedssl">
    A self-signed certificate lets the 3x-ui panel use HTTPS, so all data between your browser and the panel is encrypted instead of sent in plain text. Your browser may show a warning because the cert isn’t from a public CA, but the connection is still secure. This helps prevent man-in-the-middle (MITM) attacks. It is also much safer than using HTTP, especially when entering your password or changing VPN settings.
  ...
  <Tab title="SSH port forwarding" icon="left-right">
    SSH port forwarding (SSH tunneling) lets you securely access a service on your VPS, like the 3x-ui web panel, without opening its port to the public internet. You create an encrypted SSH connection where your PC listens on a local port (for example `2222`). The tunnel forwards all traffic to the 3x-ui port on the server (for example `46522`). When you open `http://127.0.0.1:2222/` in your browser, you’re safely talking to the panel through SSH.
    ...
    <b>Create an SSH tunnel from your PC:</b>
    ...
    You can also use this second example to connect to the server from your local machine, where the 3x-ui port is 46522:

    ```bash
       ssh -L 127.0.0.1:2222:127.0.0.1:46522 root@YOUR_SERVER_IP
    ```

    Open the URL in the browser on your local PC:
...
After logging into the 3X-UI panel, go to the Inbounds tab and click the Add Inbound button:
...
- Click the Client drop-down list and click one by one to the Email, ID, Subscription to update.
- For Flow settings select <b>xtls-rprx-vision</b>. The Flow will be visible after activating Reality for the Security setting.
...
- SNI - google.com,[www.google.com](http://www.google.com)
...
If configured correctly, you will have an entry with the protocol. Try to click the <b>+</b> plus to show client settings for export:

![inboundcreated](/assets/3x-ui-inboundcreated.png)

In the drop-down menu, click the QR code image:
...
Description of 3X-UI clients for PC: **Hiddify**, **v2rayN**, **v2RayTun**, **v2RayNG**, **Nekoray** and others.
