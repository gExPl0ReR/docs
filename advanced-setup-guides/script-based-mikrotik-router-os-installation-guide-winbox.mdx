...
5. ![Install MikroTik RouterOS](/assets/HBuvnsEfBThxg38W0vgJY_image.png "Note: Changes to KVM settings require a shutdown and power cycle to take effect.")
...
# Mount img and inject autorun.scr
loop_dev=$(losetup -f --show chr.img)
kpartx -av "$loop_dev"
sleep 1
mount "/dev/mapper/$(basename $loop_dev)p2" /mnt/ && \
...
2. Open Winbox and create a new connection:
   ...
   ![Install MikroTik RouterOS](/assets/y9B0AGVAxYeSyeZVyaDis-CxSq9rYIoBmHYZ9f10ljp-20241112-101352.png)

3. During the first login, you’ll be prompted to set a new password for added security.

![Install MikroTik RouterOS](/assets/y9B0AGVAxYeSyeZVyaDis-5QY8EKzVD1DhazfKvT1H8-20241112-102055.png)
