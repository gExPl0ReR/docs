---
title: "Connect Ubuntu 22.04 WireGuard client to AmneziaWG"
description: "Install the WireGuard client on Ubuntu 22.04 and connect to an AmneziaWG VPN server. Step-by-step setup for a secure, DPI-resistant VPN connection."
---
...
### 1. Change sources.list on the client's operating system
...
If the command doesn't show any output, execute this command:
...
* /root/awg/ - directory where the AmneziaWG client config files are located
...
Download the Amnezia WireGuard configuration file from the VPS. The `.conf` file is usually in the following format:
...
Move your configuration file to AmneziaWG’s directory. Here, wg-client1.conf is the client config you previously downloaded from the server:
