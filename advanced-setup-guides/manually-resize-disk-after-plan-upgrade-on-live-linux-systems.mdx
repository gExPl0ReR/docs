...
In the GParted window that opens, you will see several partitions. There will be vda1 and vda2 partitions. You need to add unallocated space to vda1 (root) partition but you will not be able to expand space in the /dev/vda1 (root) partition without deleting /dev/vda2 (swap). &#x20;
...
4. **Create a swap partition**. Right-click on unallocated space and select New. In the Field File system switch to Linux-swap and click Add button.
