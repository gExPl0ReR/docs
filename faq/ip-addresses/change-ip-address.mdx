...
1. **Log into your EDIS Global** [dashboard](https://manage.edisglobal.com/clientarea.php) using your credentials.
...
If 24 hours have passed, IP changes are no longer free. You can choose from two options:

| Option | Price | Details |
|---|---|---|
| Standard IP Change | **€5** | A random new IP is automatically assigned from a /24 subnet. You cannot pick the subnet. |
| Bespoke IP Change | **€10** | You get a selectable list of available IPs and can choose one. |
...
After the IP change, you will need to reinstall your system or manually configure the new IP in your OS using VNC. Once a new IP is assigned, your old IP will stop working immediately.
...
> **Note:** If you previously cancelled a service and would like to use the same IP address again, there is a chance you may find it available through the Bespoke IP Change option - provided the IP has not yet been reassigned yet.

<CtaButton label="Order Bespoke IP Change" link="https://manage.edisglobal.com/cart.php?gid=addons" />

**Note:** After the IP change, you will need to reinstall your system or manually configure the new IP in your OS using VNC. Once a new IP is assigned, your old IP will stop working immediately.

## Configure New IP After Change (common for all)

Regardless of whether the change was free or paid:

- **Reinstall the VPS** is the easiest path to activate the new IP cleanly.
- Or, **manually configure** your new IP using VNC to set network settings.
...
- **If within 24 hours**, use the free option.
- **After 24 hours**, choose between Standard (cheaper & automatic) or Bespoke (select your IP).
- Always reinstall or reconfigure after change, regardless of scenario.
