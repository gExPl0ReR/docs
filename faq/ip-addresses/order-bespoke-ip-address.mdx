---
title: Order your bespoke IPv4 address
sidebarTitle: Bespoke IP Address
description: "Hand-pick your own IPv4 addresses and IP ranges with the EDIS Global Bespoke IP Address service. Step-by-step ordering process and pricing details inside."
---

import { CtaButton } from "/snippets/cta-button.mdx";

## Bespoke IP address service

Our premium IP address product allows you to **hand-pick from a provided list of prefixes and IP addresses**. You can add up to 10 additional IPv4 addresses to your VPS. 

A one-off fee per IP address applies on top of the monthly fee for the IP address.

Choices depend on the addresses and prefixes available at the VPS location. If you need a particular number of distinct `/24` prefixes, confirm inventory before ordering. Ordinary IP allocation and standard replacements are random; they do not guarantee a different subnet.

Selecting an address does not guarantee access from a particular country or to a particular platform, residential IP classification, or a particular reputation-list status. See [connectivity and IP expectations](/commercial-vpn-hosting#connectivity-and-ip-expectations).





| Service                                                      | Price            |
| :----------------------------------------------------------- | :--------------- |
| Choose from a hand-selected list of IPs from various /24 prefixes. | **€10** one-time |
| Monthly fee for an IPv4 address                              | **€5** /month    |



## Ordering a bespoke IPv4 address

If you are interested in ordering bespoke IP addresses

* Reach out to us via messenger
* Provide the **primary IP address** or **KVM_ID** of your VPS
* Indicate the **number of additional bespoke IP addresses** you'd like to order



**We'll take care of the ordering process and send you an invoice.**
We'll provide you with a list of IP addresses in multiple prefixes available in the server location where your VPS is hosted.

Once the payment has gone through, our team will provision the IPv4 address you picked. 
After provisioning, all IP addresses will be displayed in the VPS Management Portal. 

You need to manually configure additional IP addresses in your operating system.



<CtaButton label="Be lazy" link="./order-additional-ip-address" />
