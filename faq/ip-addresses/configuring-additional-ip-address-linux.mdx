...
description: "Configure an additional IPv4 address on a Linux VPS, with examples for Debian and Ubuntu using netplan and ifupdown for persistent network settings."
---

## Configure a secondary IP in Linux

When you order an additional IP address for your VPS, it will not be added to your server's network settings automatically. You have to manually configure the IP address in your operating system's network configuration or make a clean reinstall to get your IP address configured automatically. Note that all data on your KVM will be erased during reinstall.
