---
title: "How to Investigate and Stop Unusual Bandwidth"
sidebarTitle: "Detect Unusual Bandwidth Usage"
description: "Your VPS may start consuming traffic because of hidden background processes — from rogue scripts and crypto miners to misbehaving cron jobs. Learn how to find and stop them before they drain your bandwidth."
---

# Detecting Suspicious Bandwidth Spikes in Linux Servers 🚨

Unusual bandwidth spikes often come from background activity you don’t expect — hidden tasks or processes quietly using your network, such as:
...
Spotting these early helps you prevent wasted bandwidth, slow performance, and unexpected traffic costs.
...
Start by checking which IPs or processes are using bandwidth. Two lightweight tools are iftop and nload.
...
## 2) Identify Which Process Is Behind the Traffic

Once you spot unusual traffic, use ss (modern) or netstat (optional) to find the process behind the connections.
...
For example, a process like xray-linux-amd6 with PID 28799 is making outbound connections to multiple external IPs — a strong sign of a background service or script consuming bandwidth.

![ss and netstat example screenshot](/assets/tunpn-tests.png)
💡 *ss / netstat output shows which PIDs are opening remote connections and potentially causing abnormal traffic.*
...
| ![traffic before](/assets/iftop-test.png) | ![traffic after](/assets/after-kill.png) |
|:--:|:--:|
|Traffic before stopping process.|Traffic after stopping — usage drops.|
...
## 6) Use nethogs for Per-Process Bandwidth Tracking

To continuously monitor which processes use the most bandwidth:
...
## 7) Check Your Overall Traffic Consumption

If you suspect something has already used too much bandwidth, you can review your traffic stats directly in your EDIS dashboard.  
It shows **total monthly usage**, **daily graphs**, and **refill times**.

📘 [Learn how EDIS Global meters traffic and how refills work here](https://docs.edisglobal.com/faq/traffic-bandwidth/traffic-stats-refill-explained).
...
✅ Following these steps ensures your VPS stays stable, secure, and free from hidden processes consuming bandwidth or resources.