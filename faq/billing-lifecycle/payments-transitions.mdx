...
- By default, account credit is applied automatically to new renewal invoices
...
By default, account credit is applied automatically to new renewal invoices and can also be used for new orders. Your credit balance is displayed in the control panel.
...
For approved accounts, support can configure an account-level arrangement that disables automatic credit application during scheduled invoice creation. Credit is temporarily set to zero before the billrun and restored afterward. This can be useful when combining Direct Debit with account credit. Renewal invoices are collected through the agreed Direct Debit workflow, while restored account credit can be used for new purchases with near real-time delivery.

See [Optional credit handling during invoice creation](/faq/billing-lifecycle/scheduled-billing-window#optional-credit-handling-during-invoice-creation) for timing and [the account-credit guide](/faq/billing-lifecycle/pay-invoices-from-credit-balance#can-i-disable-automatic-credit-application-to-renewal-invoices) for how to request this arrangement. Other automatic payment methods are managed separately.
