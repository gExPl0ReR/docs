...
```mermaid
flowchart TD
    A["Customer places order (Pro-forma Invoice 4004xxx)"] --> B["Customer makes payment"]
    B --> C["Payment provider confirms transaction"]
    C --> D["Billing system converts invoice (Tax Invoice 500xxxx)"]
    D --> E["Provisioning system checks for paid orders (Every 3 minutes)"]
    E --> F["VPS is provisioned & credentials sent"]
    
    subgraph Installation
        direction TB
        F --> G["Linux installation (~5 min)"]
        F --> H["Windows installation (~15 min)"]
    end
