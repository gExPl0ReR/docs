---
title: VPS Billing Workflow
description: The doc outlines the automated billing procedure for EDIS Global VPS services, including invoice generation, credit card captures, friendly and overdue reminders, service suspension, and termination.
---

## Understanding EDIS Global's Automated VPS Billing Workflow

We want to provide you with a clear overview of our automated VPS billing process, including when we generate invoices, process credit card payments, send reminders, and what occurs if an invoice remains unpaid.

### Billing Timeline

1. **Invoice Generation**

   * Generated 7 days before the due date

2. **Credit Card Processing**

   * Automated capture 4 days before the due date

3. **Payment Reminders**

   * First friendly reminder: 2 days before the due date

   * Overdue reminders: Sent 1, 2, and 3 days after the due date

4. **Service Actions**

   * Service Suspension: Occurs 3 days after the due date

   * Service Termination: Takes place 8 days after the due date (**all data is permanently wiped**)

### Service Recovery Available

If your VPS has been terminated, but your invoice is still showing **UNAPID** in the portal, there may be an opportunity to restore your VPS including your data.&#x20;

![invoice status unpaid, service can be recovered](/invoice-status-unpaid.webp)

1. Make the payment immediately

2. Contact our support team to notify us of your payment



### Service Recovery No Longer Available

If your VPS has been terminated, and your invoice shows CANCELLED in the portal, your VPS has finally been deleted and there is no option to recover the service.

![invoice status cancelled, service can no longer be recovered](/invoice-status-cancelled.webp)



### Billing Timeline Flowchart

```mermaid
flowchart TD
    A["🧾 Invoice Generated<br>(7 days before due)"] --> B["💳 Credit Card Processing<br>(4 days before due)"]
    B --> C["📩 First Friendly Reminder<br>(2 days before due)"]

    subgraph overdue["🚨 Overdue Reminders"]
        direction TB
        C1["📢 1 Day Overdue"]
        C2["📢 2 Days Overdue"]
        C3["📢 3 Days Overdue"]
    end
    C --> C1 --> C2 --> C3

    C3 --> D["⛔ Service Suspension<br>(3 days overdue)"]
    D --> E["❌ Service Termination & Service Deletion<br>(8 days overdue)"]

    subgraph recovery["🔄 Recovery Possible"]
        direction TB
        R1["💰 Make Immediate Payment"]
        R2["📞 Contact Support"]
        R3["✅ Possible Data Restoration"]
    end

    subgraph no_recovery["⚠️ No Recovery"]
        direction TB
        X[""🛑 Data Permanently Lost""]
    end

    E -.->|Invoice Status Unpaid| R1 --> R2 --> R3
    E -.->|Invoice Status Cancelled| X
```

<Note>
  **Client Responsibility Notice**

  VPS customers are solely responsible for:

  * Managing their virtual machines

  * Creating regular backups of their servers

  * Securing all important data

  EDIS Global strongly recommends maintaining current backups of all critical information.
</Note>