---
title: "Order API Endpoints for Automating VPS Provisioning"
sidebarTitle: "Order API Overview"
description: "Reference all Order API endpoints for ordering, configuring, upgrading, and canceling EDIS Global VPS services programmatically over HTTPS with JSON."
---
...
<Warning>
  **Production API — no test environment**

  EDIS Global does not provide a sandbox or test environment. Submitting a valid order to `POST /kvm/v2/add/order` creates a real VPS order and invoice. Available account credit is applied by default unless you explicitly set `"applycredit": false` inside the `order` JSON.

  Discovery requests, such as retrieving plans or locations, do not place orders.
</Warning>
