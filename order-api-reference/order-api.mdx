...
<Tabs>
  <Tab title="Simple Guide">
    ...
    - plan IDs → `pid`
    - OS IDs → `os`
        - addon IDs → RAM, vCPU, IPs and Drive Size Boost

        Always use the addon IDs returned for the selected product and location. Do not hard-code IDs from documentation examples because configurable-option IDs can differ or change.
    ...
    - optional:
      ...
      - IPs: up to 10 additional IPv4 addresses
      - Drive Size Boost: +10, +20, +30, +40, +50, +100, +200, +300 or +400 GB

    Everything is submitted using the option ID returned by `POST /kvm/v2/get/products`. Drive Size Boost is added to the plan's included storage, so a 100 GB plan with the 400 GB option provides a 500 GB virtual drive.
